    ...
    class Redir:
        ...
        def sendline(self, data):
            if not self.stop_flag:
                try:
                    ...
                    # the browser has already closed the connection and performed a redirect
                    self.stop_flag = True
        ...
        def run(self,):
            ...
            if data:
                ...
                self.sendline( b"Redir: close")
                self.sendline( b"Cache-control: private")
                self.sendline( b"")
                self.sendline( b"<html><body>Encryption Required <a href='" 
                               + TARGET + b"'>" + TARGET + b"</a></body></html>")
                self.sendline( b"")
            self.socket.close
            dt = datetime.now().strftime("%d/%b/%Y %H:%M:%S")
            self.logger and self.logger.info( f'{self.ip} - - [{dt}] to "{TARGET.decode()}"' )
    ...
    class DualSocket:
        ...
        def accept(self):
            ...
            if conn.recv(1, socket.MSG_PEEK) == b"\x16":
                ...
                Redir(conn, addr, self.host, self.port, self.logger).run
